  ...
  <div style="height:100vh; display: flex; justify-content: center; align-items: flex-start;">
    <Card style="width: 360px; margin-top: 100px;">
      ...
      <div v-if="step==1" style="margin: 24px 10px 10px;">
        <Form ref="user" :model="user" :rules="userRules">
          ...
          <FormItem prop="password">
            <Input type="password" maxlength="16" v-model="user.password" placeholder="密码">
      ...
      <div v-if="step==2" style="margin: 24px 10px 10px;">
        ...
        <Button style="width: 100%;" type="primary" :loading="loading2" @click="handleSubmit2">注册</Button>
      </div>
  ...
  methods: {
    handleSubmit1 () {
      ...
      this.$refs['user'].validate((valid) => {
        if (valid) {
          this.$post('/tourist/registerStep1', this.user)
            .then(data => {
              if (data.data.code === 0) {
                this.step = 2
              } else {
    ...
    handleSubmit2 () {
      ...
      this.$refs['verification'].validate((valid) => {
        if (valid) {
          ...
          this.$post('/tourist/registerStep2', this.user)
            .then(data => {
              if (data.data.code === 0) {
                this.step = 3
                setTimeout(() => {
